...
-- Хранимая функция для применения скидок
...
BEGIN
    ...
    -- Применение скидки и возврат измененных строк
    RETURN QUERY
    UPDATE games g
    ...
    RETURNING 
        g.title,
        g.price / (1 - p_discount_percent::DECIMAL / 100) AS old_price,
        g.price AS new_price;
...
-- Пример использования функции скидок
-- SELECT * FROM apply_discount(1, 10); -- Скидка 10% для Sega (ID=1)
-- SELECT * FROM apply_discount(2, 15); -- Скидка 15% для Dendy (ID=2)
